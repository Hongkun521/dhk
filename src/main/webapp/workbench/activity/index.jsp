...
<script type="text/javascript">
	...
	$(function(){
				...
				success:function (data) {
					...
					*/
					if(data){
						...
						$("#activityPage").bs_pagination('getOption', 'rowsPerPage')   维持已经设置好的每页展现的记录数
						这两个参数不需要我们进行任何的修改操作，直接用就可以
						*/
		...
		//为删除按钮绑定事件，执行市场活动的删除事件
		$("#deleteBtn").click(function () {
			//找到复选框中所有带√的jQuery复选框对象
		...
		$("#updateBtn").click(function () {
			//在项目中，一定是按照先添加，再修改的这种顺序，所以，为了节省开发时间，修改操作一般都是copy操作
			$.ajax({
	...
	/*
	  对于所有的关系型数据库，做前端分页相关操作的基础组件，就是pageNo, 和pageSize
	  pageNo:页码
	  pageSize:每页展现的记录数
	  pageList方法：就是ajax请求到后台，从后台取得最新的市场活动信息的列表数据
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 5px;">
				<div class="btn-group" style="position: relative; top: 18%;">
					<%--点击创建按钮观察两个属性和属性值
						data-toggle：表示触发该按钮，将打开一个模态窗口
						data-target：表示要打开哪个窗口，通过#id的形式找到该窗口

						但是这样做是有问题的，问题在于没有办法对按钮功能进行扩充

						所以未来的实际项目开发，对于触发模态窗口的操作，一定不能写死在元素中，应该由我们自己写js代码来操作
					--%>
